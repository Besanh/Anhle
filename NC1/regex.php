...
<div style="margin-top:10px;">
    ...
    <?php 
        /******************************************************
        * LAY TU BANG RECHARGE TN CUA "OCB" THOA DKIEN NAP TIEN
        * RECHARGE LUU TN CUA "OCB"
        * RP_FINAL LUU TN CUA "OCB" THOA DKI
        ******************************************************/
    
    
    
        // BU?C 2: TÌM T?NG S? RECORDS
        $result = mysqli_query($con, 'select count(id) as total from `nghia_nhantin`.`recharge`');
        ...
        $result = mysqli_query($con, "SELECT * FROM `nghia_nhantin`.`recharge` WHERE phone = '+84976764205' ORDER BY id DESC LIMIT $start, $limit");
    ...
    <table class="table table-striped" style="text-align: center;" border="1">
      <thead style="font-weight:bold; background-color: #1ab188;">
            <tr>
                ...
                <th width="30%" style="text-align: center;">History recharge</th>
                <th width="60%" style="text-align: center;">Content</th>
            </tr>
        </thead>
        <tbody>
            <?php
