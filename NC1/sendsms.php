<?php
    ...
    if(isset($_POST['bt_submit'])){
        if(!isset($_POST['check'])){
            ...
            header("Location: gui-tin-nhan.html");
        }
        else{
                    ...
                    for($i = 0; $i < count($port); $i++){
                        for($j = 0; $j < count($arr); $j++){
                            if($port[$i] == $j){
                                ...
                                // GUI TN LA TRU TIEN LIEN
                                $fund = $_SESSION['fund_result'];
                                if($fund == 0){
                                  $_SESSION['message'] = "Your account has run out of funds";
                                  header("Location: gui-tin-nhan.html");
                                  exit;
                                }
                                else{
                                  $fund_result = $_SESSION['fund_result']-100;
                                  ...
                                  if($status == "FAILED"){
                                      $sql = "INSERT INTO `inbsent`(id_user, port, date, time, sender, receiver, status, note, content)
                                              VALUES ('$a', '$p', CURRENT_DATE(), CURRENT_TIME(), '$sender', '$receiver', '$status', '$note', '$content')";
                                      $conn = mysqli_query($con, $sql);
                                      $sql2 = "INSERT INTO `sms`(`id_user`, `phone`, `port`, `status`) 
