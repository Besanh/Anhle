...
<div style="margin-top:10px;">
    ...
    <?php 
        ...
        $result = mysqli_query($con, 'select count(id) as total from sms');
        ...
        $result = mysqli_query($con, "SELECT port, phone, SUM(A.count + A.count1 + A.count2) AS number, GROUP_CONCAT(A.count SEPARATOR ',') as delivered, 
                              GROUP_CONCAT(A.count1 SEPARATOR ',') as sent_ok, 
                              GROUP_CONCAT(A.count2 SEPARATOR ',') as failed 
                              FROM (SELECT port, GROUP_CONCAT(DISTINCT phone) AS phone, 
                                    SUM(CASE WHEN Status = 'DELIVERED' THEN 1 ELSE 0 END) as count, 
                                    SUM(CASE WHEN Status = 'SENT_OK' THEN 1 ELSE 0 END) as count1, 
                                    SUM(CASE WHEN Status = 'FAILED' THEN 1 ELSE 0 END) as count2 
                                    FROM sms GROUP BY port) A 
                              GROUP BY A.port
                              ORDER BY A.port ASC LIMIT $start, $limit");
